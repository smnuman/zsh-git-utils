...
This toolkit assumes a `$ZDOTDIR` repo where key subfolders (e.g. `zsh`, `brew`, `prompt`) may be their **own Git repos** and connected to the parent via **Git submodules**.
...
- **🌐 Support both GitHub and GitLab providers with retry/fallback logic**

> **New:** All functions are individual executables in `$ZDOTDIR/lib/git-utils/` and loaded via PATH (fast on-demand loading!).

---

## 🚀 Quick Setup

**This toolkit uses PATH-based executables for fast shell startup:**

1. **Source environment and toolkit in your shell config** (`$ZDOTDIR/.zshenv` or `$ZDOTDIR/.zshrc`):
   ```bash
   source $ZDOTDIR/lib/zsh-initgit.zsh
   source $BREWDOTS/.env
   ```
...
**What changed:**
- ✅ Functions are now individual executable files (not sourced functions)
- ✅ Loaded on-demand via PATH (90%+ faster shell startup)
- ✅ Environment variables managed via `$BREWDOTS/.env` and related files
- ✅ Original monolithic file archived for reference
...
- **GPG** (optional) for sharing encrypted repos:
  ...
  brew install gnupg
  ```
- Zsh completion (optional) for nicer UX; completions are cached for performance (see "Tab Completion").

---

## 🌐 Multi-Provider Support

Set your Git provider using environment variables in `$BREWDOTS/.env` or `$ZDOTDIR/.zshenv`:

```bash
export GIT_PROVIDER="github"    # or "gitlab" (default: github)
export GITHUB_USER="yourusername"
export GITLAB_USER="yourusername"  # optional, falls back to GITHUB_USER if unset
```
...
**Note:** If `GITLAB_USER` is unset or empty, it will fallback to `GITHUB_USER` to ensure smooth operation.

⸻

## 🔧 Core Helper Executables

Each utility is an individual executable script in the `git-utils` PATH directory, providing provider-agnostic functionality:

### `grepo_name [dir] [custom_name]`
Generate or validate standardized repository names.
- Auto-generates from directory structure (parent-child format)
- Validates custom names (alphanumeric/hyphens, max 40 chars)

### `githost`
Returns the current Git provider (`github` or `gitlab`) based on `GIT_PROVIDER`.

### `gurl <username> <repo_name> [provider]`
Generates the SSH remote URL for the specified provider:
- `git@github.com:user/repo.git` (GitHub)
- `git@gitlab.com:user/repo.git` (GitLab)

### `gitcli <command> [provider]`
Returns the CLI command for the provider (`gh` or `glab`).

### `gituser [provider]`
Returns the username from `GITHUB_USER` or `GITLAB_USER` environment variables.
...
### `git-genignore [target_dir]`
Generate a standard `.gitignore` file with sensible defaults for any Git repository.

**Features:**
- Prompts before overwriting existing files
- Includes patterns for OS files, editors, logs, dependencies, temp files, and secrets
- Adds a custom section for project-specific patterns
...
**Example:**
```bash
cd $ZDOTDIR/myproject
git-genignore              # Generate in current directory
git-genignore ~/myrepo     # Generate in specific directory
```

### `gsub-genignore [target_dir]`
Generate a `.gsubignore` file to specify which folders should NOT be converted to submodules.

**Features:**
- Scans current directory and finds common ignorable patterns
- Shows both ignored and non-ignored folders
- Highlights which folders are already git repositories
- Supports custom patterns via manual editing

**Example:**
```bash
cd $ZDOTDIR
gsub-genignore    # Create .gsubignore with detected patterns
gsub-all "Initial commit"  # Respects .gsubignore
```

⸻

## 🔐 Encryption Executables

Encryption utilities have been renamed and modularised:
...
Sets up git-crypt encryption with sensible defaults for sensitive files.
...
### `gencrypt_check <file>`
Checks if a specific file is encrypted by git-crypt.

### `gsecrets [dir]`
Scans repository for potential unencrypted secrets. Checks for common patterns like API keys, passwords, tokens.
...
# Verify encryption
gencrypt_check .env              # Check if .env is encrypted
gencrypt_check config.local.json # Check custom file
gsecrets                         # Scan for any exposed secrets
```
...
```
Create and push a new Git repo to GitHub/GitLab from the current folder.
If the remote doesn't exist, it is created via `gh` or `glab`.
...
```
Initialise `<folder>` as a submodule, push its repo to GitHub/GitLab (if needed), and update the parent repo pointer.
...
These are used internally by the other modules:
...
If completions are enabled, drop `_gutils` into `$ZDOTDIR/completions/` and load it:
```sh
$ZDOTDIR/completions/_gutils
```
                - #compdef
                - gsub
                - grepo
...
Completions are cached automatically for faster shell startup.
...
```bash
$ZDOTDIR/docs/zsh/git/TEST.SUITE.06.autoConflictResolution.zsh
```
...
Your `$ZDOTDIR/docs/setup.md` can refer to:
...
If you encounter issues or need to reinitialise your environment:
...
- Re-initialize git-crypt hooks and encryption:
  ```bash
  gencrypt_setup
  ```

- Reauthenticate GitLab CLI (if needed):
  ```bash
  glab auth login
  ```
...
- Verify environment variables and PATH:
  ```bash
  echo $GIT_PROVIDER $GITHUB_USER $GITLAB_USER
  which grepo
  ```
